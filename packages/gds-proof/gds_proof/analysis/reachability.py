"""Predicate-guarded reachability analysis.
...
Output symbols of blocks with unwired ports are included in the analysis
if those symbols appear in declared invariants.  The composed model is
itself a block whose open inputs and all outputs define its external
interface.

Three layers of analysis
------------------------

1. **Single-step reachability** — for each (invariant I_j, block k) pair:
   ...
   Contrast with ``analyze_invariants``, which uses only the individual
   pre-state invariant as an antecedent.
...
2. **Predicate sufficiency** — predicates alone, without invariants:
   ...
   on their own, regardless of starting state.  When True, the block
   is self-certifying for that invariant.

3. **Multi-step induction** — if ALL single-step results are PROVED, then
   by induction no reachable state violates any invariant.  This is the
   strongest possible result: global inductive safety.

Relationship to predicates as pullback guards
---------------------------------------------
When predicates are constructed as ``check(f(x_prev, u))`` (pullback of a
post-state condition), predicate sufficiency tends to be trivially PROVED
for the corresponding local invariant — the guard was designed precisely to
enforce it.  The interesting proofs are the cross-block cases: does block
k's local guarantee help preserve invariant I_j that block k did not
directly design for?
"""
...
class SingleStepVerdict(StrEnum):
    """Verdict for single-step reachability of one (invariant, block) pair."""
...
class SingleStepResult(BaseModel):
    """Single-step reachability for one (invariant, block) pair.
...
class PredicateSufficiencyResult(BaseModel):
    """Predicate-only reachability for one (invariant, block) pair.
...
class ReachabilityAnalysisResult(BaseModel):
    """Complete three-layer reachability analysis for a model."""
...
def _analyze_single_step(
    ...
    # Build antecedent: all invariants ∧ all predicates
...
def _analyze_predicate_sufficiency(
    ...
    Proof obligation (no invariant conjunction in antecedent):
    ...
    if pred_conjunction is None:
        # No predicates — cannot be sufficient
...
def _derive_multi_step(single_step_results: list[SingleStepResult]) -> MultiStepResult:
    ...
    If ALL single-step pairs are PROVED, the system is inductively safe:
    no admitted transition sequence can reach an invariant-violating state.
    """
...
def analyze_reachability(model: ProofableModel) -> ReachabilityAnalysisResult:
    """Run three-layer predicate-guarded reachability analysis.

    Parameters
    ----------
    model:
        Any object satisfying ``ProofableModel``.
